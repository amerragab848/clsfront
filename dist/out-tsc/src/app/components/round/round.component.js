...
let RoundComponent = class RoundComponent {
    constructor(_roundSrv, router, _toastSrv, _labSrv, _courseSrv, _branchSrv, _deliveryTypeSrv, _courseTypeSrv, _instructorSrv, datepipe) {
        this._roundSrv = _roundSrv;
        this.router = router;
        this._toastSrv = _toastSrv;
        this._labSrv = _labSrv;
        this._courseSrv = _courseSrv;
        this._branchSrv = _branchSrv;
        this._deliveryTypeSrv = _deliveryTypeSrv;
        this._courseTypeSrv = _courseTypeSrv;
        this._instructorSrv = _instructorSrv;
        this.datepipe = datepipe;
        this.instructors = [];
        this.selectedInstructors = [];
        this.roundWithDetails = {
            roundDetils: null,
            instructors: null
        };
        this.round = {};
    ...
    GetRoundDetails(roundId) {
        this._roundSrv.GetRoundDetails(roundId).subscribe((data) => {
            this.roundWithDetails = data.result;
            console.log(this.roundWithDetails);
        });
    }
    GetRoundById(roundId) {
        this._roundSrv.GetRoundById(roundId).subscribe((data) => {
            this.GetAllLabs();
    ...
    GetAllLabs() {
        this._labSrv.GetLabs().subscribe((data) => {
            this.labs = data.result;
        });
    }
    GetCourseRounds() {
        this._roundSrv.GetCourseRounds(this.router.snapshot.params.id).subscribe((data) => {
            this.rounds = data.result;
            console.log(data);
        });
    }
    GetCourse() {
        this._courseSrv.GetCoursees().subscribe((data) => {
    ...
    ExecuteRound(roundId) {
        this._roundSrv.ExecuteRound(roundId).subscribe((data) => {
            if (data.code === 200) {
                this._toastSrv.success("", "Saved Successfully");
                this.GetCourseRounds();
            }
    ...
    GetRoundsTimeTable() {
        this._roundSrv.GetRoundsTimeTable(this.round.labId, parseFloat(this.router.snapshot.params.id), this.round.startDate, this.round.endDate).subscribe((data) => {
            this.timeTable = data;
            console.log(data);
        });
    }
    DeleteRound(roundId) {
        this._roundSrv.DeleteRound(roundId).subscribe((data) => {
            if (data.code === 200) {
    ...
    CalculateTotalSession() {
        var rLength = this.roundWeek.length;
        var rc = 0;
    ...
    CalculateEndDate() {
        ...
        this._roundSrv.GetRoundEndDate(this.round.startDate, parseFloat(weeks.toString())).subscribe((data) => {
            console.log(data);
    ...
    GetBranches() {
        this._branchSrv.GetBranches().subscribe((data) => {
    ...
    GetDeliveryTypes() {
        this._deliveryTypeSrv.GetDeliveryTypes().subscribe((data) => {
            this.deliveryTypes = data.result;
        });
    }
    GetCourseTypes() {
        this._courseTypeSrv.GetCourseTypes().subscribe((data) => {
            this.courseTypes = data.result;
        });
    }
    GetInstructors() {
        this._instructorSrv.GetInstructors().subscribe((data) => {
